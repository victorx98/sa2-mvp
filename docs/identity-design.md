...
## 八、Identity 域服务设计 (DDD)

本章节遵循领域驱动设计（DDD）原则，对 Identity 域的核心服务、接口和数据传输对象（DTO）进行详细设计。

### 8.1 领域层 (Domain Layer)

领域层是业务逻辑的核心，封装了最纯粹的业务规则和状态。

#### 8.1.1 领域实体与服务 (Domain Entities and Services)

```typescript
// src/domains/identity/user/user-interface.ts (Entity Interface)
import type { DrizzleTransaction } from "@shared/types/database.types";
import * as schema from "@infrastructure/database/schema"; // Assuming schema is available for $inferSelect
...
export interface User {
  ...
  createdTime?: Date;
  modifiedTime?: Date;
  roles?: string[]; // Roles are string IDs
}

// src/domains/identity/user/user-interface.ts (Create User Input Interface)
export interface CreateUserInput {
  ...
  cnNickname?: string;
  gender?: string;
  status?: string; // Can be 'active' | 'inactive' | 'pending_verification'
  country?: string;
}

// src/domains/identity/user/user-service.ts (Domain Service)
// This class implements IUserService (defined in user-interface.ts)
class UserService {
  // 构造函数注入数据库连接
  constructor(db: any /* NodePgDatabase<typeof schema> */) {} // Using 'any' to avoid full import path in doc
   ...
   */
  public findByIdWithRoles(id: string): Promise<User | null>;
   ...
   * Create a new user
   */
  public create(user: CreateUserInput, tx?: DrizzleTransaction): Promise<User>;

  /**
   * Create a new user with roles in a transactional way
   */
  public createWithRoles(user: CreateUserInput, roles: string[], tx?: DrizzleTransaction): Promise<User>;

  /**
   * Assign roles to user
   */
  public authorizeRoles(userId: string, roles: string[], tx?: DrizzleTransaction): Promise<string[]>;

  /**
   * Get user roles by user ID
   */
  public getRolesByUserId(userId: string, tx?: DrizzleTransaction): Promise<string[]>;

  /**
   * Update user
   */
  public update(id: string, user: Partial<CreateUserInput>, tx?: DrizzleTransaction): Promise<User>;

  // Private helper method for mapping database records to User entity
  private mapToUser(record: any /* typeof schema.userTable.$inferSelect */): User;
}

// Note: The 'RoleService' class is not present in the provided 'src/domains/identity/user/' directory.
// Role management is handled by string IDs within UserService.
```

### 8.2 应用层 (Application Layer)

应用层负责编排领域对象和领域服务，以完成具体的业务用例（Use Cases）。

#### 8.2.1 命令服务 (Command Services - Write Operations)

```typescript
// src/application/commands/auth/auth-command.service.ts
class AuthCommandService {
  /**
   * 协调用户注册的完整流程，包括创建认证用户和业务用户。
   * 内部处理补偿逻辑。
   */
  public registerNewUser(dto: RegisterUserDto): Promise<User>;

  /**
   * 为指定用户分配一个新角色。
   */
  public assignRoleToUser(userId: string, roleName: string): Promise<void>;

  /**
   * 更新用户的核心业务信息。
   */
  public updateUserProfile(userId: string, dto: UpdateUserProfileDto): Promise<User>;
}
```

#### 8.2.2 查询服务 (Query Services - Read Operations)

```typescript
// src/application/queries/user/user-query.service.ts
class UserQueryService {
  /**
   * 根据用户ID获取完整的用户聚合根（包含角色信息）。
   */
  public getUserWithRoles(userId: string): Promise<User | null>;
   ...
   * 获取拥有特定角色的所有用户列表。
   */
  public findUsersByRole(roleName: string): Promise<User[]>;
}
```

### 8.3 基础设施层 (Infrastructure Layer)

基础设施层提供与外部系统（如数据库、认证服务）交互的具体实现。

#### 8.3.1 认证服务适配器 (Auth Service Adapter)

适配器用于隔离外部认证服务（Supabase Auth）的复杂性。
...
interface IAuthServiceAdapter {
  /**
   * 在外部认证提供商处创建用户。
   * @returns {Promise<{id: string}>} 返回外部系统中的用户ID。
   */
  createAuthUser(email: string, password: string): Promise<{ id: string }>;

  /**
   * 从外部认证提供商处删除用户（用于补偿操作）。
   */
  deleteAuthUser(id: string): Promise<void>;

  /**
   * 使用邮箱密码进行登录认证。
   * @returns {Promise<{ accessToken: string; userId: string }>} 返回令牌和用户ID。
   */
  signInWithPassword(email: string, password: string): Promise<{ accessToken: string; userId: string }>;

  /**
   * 验证访问令牌的有效性。
   * @returns {Promise<{ userId: string }>} 返回令牌对应的用户ID。
   */
...
### 8.4 数据传输对象 (Data Transfer Objects - DTOs)

DTOs 是应用层与外部（如API层）交互的数据契约。

```typescript
// src/api/dto/request/register-user.dto.ts
class RegisterUserDto {
  email: string;
  password: string;
  firstName: string;
  lastName: string;
  role: 'student' | 'mentor' | 'counselor';
}

// src/api/dto/request/update-user-profile.dto.ts
...
// src/api/dto/response/user-profile.dto.ts
...
### 8.5 关联的 Profile 域服务设计

为了保持领域模型的清晰，用户的核心身份（Identity）与不同角色的详细档案（Profile）分离。`Profile` 域负责管理 `student`, `mentor`, `counselor` 的具体业务数据。

#### 8.5.1 领域服务 (Domain Services)

```typescript
// src/domains/profile/student/student-profile.service.ts
class StudentProfileService {
  ...
  // 仓储方法 (合并自 IProfileRepository)
  public findByUserId(userId: string): Promise<StudentProfileService | null>;
  public save(profile: StudentProfileService): Promise<StudentProfileService>;
}

// src/domains/profile/mentor/mentor-profile.service.ts
class MentorProfileService {
  ...
  // 仓储方法 (合并自 IProfileRepository)
  public findByUserId(userId: string): Promise<MentorProfileService | null>;
  public save(profile: MentorProfileService): Promise<MentorProfileService>;
}

// src/domains/profile/counselor/counselor-profile.service.ts
class CounselorProfileService {
  id: string;
  userId: string; // Foreign Key to User.id
  specialization?: string;
  // ... other counselor-specific fields

  // 仓储方法 (合并自 IProfileRepository)
  public findByUserId(userId: string): Promise<CounselorProfileService | null>;
  public save(profile: CounselorProfileService): Promise<CounselorProfileService>;
}
```

#### 8.5.2 应用层 (Application Layer)

```typescript
// src/application/commands/profile/profile-command.service.ts
class ProfileCommandService {
  /**
   * 创建或更新学生的档案信息。
   * 在用户注册后或后续信息补充时调用。
   */
  public createOrUpdateStudentProfile(userId: string, dto: UpdateStudentProfileDto): Promise<StudentProfile>;

  /**
   * 创建或更新导师的档案信息。
   */
  public createOrUpdateMentorProfile(userId: string, dto: UpdateMentorProfileDto): Promise<MentorProfile>;

  /**
   * 创建或更新咨询师的档案信息。
   */
  public createOrUpdateCounselorProfile(userId: string, dto: UpdateCounselorProfileDto): Promise<CounselorProfile>;
}

// src/application/queries/profile/profile-query.service.ts
class ProfileQueryService {
  /**
   * 根据用户ID获取学生档案。
   */
  public getStudentProfile(userId: string): Promise<StudentProfile | null>;

  /**
   * 根据用户ID获取导师档案。
   */
  public getMentorProfile(userId: string): Promise<MentorProfile | null>;

  /**
   * 根据用户ID获取咨询师档案。
   */
  public getCounselorProfile(userId: string): Promise<CounselorProfile | null>;
}
```
...
```typescript
// src/api/dto/request/update-student-profile.dto.ts
class UpdateStudentProfileDto {
  school: string;
  major: string;
}

// src/api/dto/request/update-mentor-profile.dto.ts
...
// src/api/dto/request/update-counselor-profile.dto.ts
class UpdateCounselorProfileDto {
  specialization: string;
}
```
