...
@Injectable()
export class GetStudentGapAnalysisSessionsUseCase {
  constructor(
    @Inject(GAP_ANALYSIS_QUERY_REPOSITORY)
    private readonly gapAnalysisQueryRepository: IGapAnalysisQueryRepository,
  ) {}

  async execute(studentId: string, filters?: GapAnalysisQueryDto): Promise<GapAnalysisReadModel[]> {
    return this.gapAnalysisQueryRepository.getStudentSessions(studentId, filters);
  }
}
