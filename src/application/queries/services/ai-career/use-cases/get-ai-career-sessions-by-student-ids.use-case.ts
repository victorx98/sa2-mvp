...
@Injectable()
export class GetAiCareerSessionsByStudentIdsUseCase {
  constructor(
    @Inject(AI_CAREER_QUERY_REPOSITORY)
    private readonly aiCareerQueryRepository: IAiCareerQueryRepository,
  ) {}

  async execute(studentIds: string[], filters?: AiCareerQueryDto): Promise<AiCareerReadModel[]> {
    return this.aiCareerQueryRepository.getSessionsByStudentIds(studentIds, filters);
  }
}
