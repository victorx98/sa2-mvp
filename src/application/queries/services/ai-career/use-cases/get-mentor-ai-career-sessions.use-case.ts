...
@Injectable()
export class GetMentorAiCareerSessionsUseCase {
  constructor(
    @Inject(AI_CAREER_QUERY_REPOSITORY)
    private readonly aiCareerQueryRepository: IAiCareerQueryRepository,
  ) {}

  async execute(mentorId: string, filters?: AiCareerQueryDto): Promise<AiCareerReadModel[]> {
    return this.aiCareerQueryRepository.getMentorSessions(mentorId, filters);
  }
}
