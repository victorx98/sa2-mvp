...
@Injectable()
export class GetMentorCommSessionsUseCase {
  constructor(
    @Inject(COMM_SESSION_QUERY_REPOSITORY)
    private readonly commSessionQueryRepository: ICommSessionQueryRepository,
  ) {}

  async execute(mentorId: string, filters?: CommSessionQueryDto): Promise<CommSessionReadModel[]> {
    return this.commSessionQueryRepository.getMentorSessions(mentorId, filters);
  }
}
