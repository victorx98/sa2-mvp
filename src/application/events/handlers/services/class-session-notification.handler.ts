...
Powered by SuperAcademy`;
  ...
  /**
   * Send failure notification to counselors
   * Common logic for create/update/cancel failures
   * Note: Class sessions may have multiple counselors
   * 
  ...
  private async sendFailureNotification(
    ...
    try {
      // Get counselor emails (may be multiple)
      const counselorEmails = payload.counselorIds && payload.counselorIds.length > 0
        ? await Promise.all(
            payload.counselorIds.map((id: string) => this.getUserEmail(id).catch(() => null))
          ).then(emails => emails.filter((email): email is string => email !== null))
        : [];

      if (counselorEmails.length === 0) {
        this.logger.warn(`No counselor emails found for session ${payload.sessionId}, skipping notification`);
        return;
      }
      ...
      // For class sessions, we may have multiple students
      const studentNames = payload.studentIds && payload.studentIds.length > 0
        ? await Promise.all(
            payload.studentIds.map((id: string) => this.userService.getDisplayName(id))
      ...
      // Send immediate notification to all counselors
      ...
      this.logger.log(`Sent ${operation} failure notification to ${counselorEmails.length} counselor(s) for session ${payload.sessionId}`);
    } catch (error) {
      this.logger.error(
