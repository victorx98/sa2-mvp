...
import { DrizzleAiCareerRepository } from '@domains/services/sessions/ai-career/infrastructure/repositories/ai-career.repository';

/**
 * AI Career Notification Handler
 * 
 * Handles notification and calendar operations for AI career sessions
 ...
 */
@Injectable()
export class AiCareerNotificationHandler {
  private readonly logger = new Logger(AiCareerNotificationHandler.name);
  ...
  constructor(
    ...
    private readonly aiCareerRepository: DrizzleAiCareerRepository,
   ...
   */
  @OnEvent(AiCareerSessionMeetingOperationResultEvent.eventType)
  @HandlesEvent(
    AiCareerSessionMeetingOperationResultEvent.eventType,
    AiCareerNotificationHandler.name,
  )
  async handleMeetingOperationResult(
    event: AiCareerSessionMeetingOperationResultEvent,
  ): Promise<void> {
  ...
  private async handleCreateSuccess(payload: any): Promise<void> {
    ...
    try {
      ...
      const session = await this.aiCareerService.getSessionById(payload.sessionId);
      const sessionTitle = session?.getTitle() || `AI Career ${new Date(payload.scheduledAt).toLocaleDateString()}`;
      // Get user emails
      ...
      if (calendarEnabled) {
        try {
          ...
          const eventId = await this.feishuCalendarService.createEvent({
            summary: `AI Career Session: ${mentorName}/${studentName}`,
          ...
          await this.aiCareerRepository.update(session);
      ...
      await this.notificationQueueService.scheduleReminders({
        ...
        sessionInfo: {
          ...
          sessionType: 'AI Career Counseling Session',
