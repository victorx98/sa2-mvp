...
import { DrizzleCommSessionRepository } from '@domains/services/comm-sessions/infrastructure/repositories/comm-session.repository';

/**
 * Comm Session Notification Handler
 * 
 * Handles notification and calendar operations for internal communication sessions
 ...
 */
@Injectable()
export class CommSessionNotificationHandler {
  private readonly logger = new Logger(CommSessionNotificationHandler.name);
  ...
  constructor(
    ...
    private readonly commSessionRepository: DrizzleCommSessionRepository,
   ...
   */
  @OnEvent(CommSessionMeetingOperationResultEvent.eventType)
  @HandlesEvent(
    CommSessionMeetingOperationResultEvent.eventType,
    CommSessionNotificationHandler.name,
  )
  async handleMeetingOperationResult(
    event: CommSessionMeetingOperationResultEvent,
  ): Promise<void> {
  ...
  private async handleCreateSuccess(payload: any): Promise<void> {
    ...
    try {
      ...
      const session = await this.commSessionService.getSessionById(payload.sessionId);
      const sessionTitle = session?.getTitle() || `Communication Session ${new Date(payload.scheduledAt).toLocaleDateString()}`;

      // Get user emails (use counselorId from payload, consistent with gap-analysis)
      const counselorEmail = payload.counselorId
        ? await this.getUserEmail(payload.counselorId)
        : undefined;
      const mentorEmail = payload.mentorId
        ? await this.getUserEmail(payload.mentorId)
        : undefined;
      ...
      if (calendarEnabled) {
        try {
          ...
          // Add mentor or counselor as attendee
          if (mentorEmail && mentorName) {
            attendees.push({ email: mentorEmail, displayName: mentorName, isOptional: false });
          }
          ...
          const eventId = await this.feishuCalendarService.createEvent({
            summary: `Communication Session: ${mentorName || counselorName || 'Staff'}/${studentName}`,
          ...
          await this.commSessionRepository.update(session);
      ...
      await this.notificationQueueService.scheduleReminders({
        ...
        sessionInfo: {
          ...
          sessionType: 'Communication Session',
  ...
  private async handleCreateFailed(payload: any): Promise<void> {
    ...
    await this.sendFailureNotification('create', payload);
  }
...
Powered by SuperAcademy`;
   ...
   * Note: Comm sessions may have mentor OR counselor (not both)
   * 
  ...
  private async sendFailureNotification(
    ...
    try {
      // Get counselor email (prefer createdByCounselorId, fallback to counselorId)
      const counselorId = payload.createdByCounselorId || payload.counselorId;
      const counselorEmail = counselorId
        ? await this.getUserEmail(counselorId).catch(() => null)
        : null;
