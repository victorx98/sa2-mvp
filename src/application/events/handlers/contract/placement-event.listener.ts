...
import { getServiceTypeFromApplicationType, ApplicationType } from "@domains/contract/utils/application-type-mapper";
...
export class PlacementEventListener {
   ...
   */
  @OnEvent(JobApplicationStatusChangedEvent.eventType)
  @HandlesEvent(JobApplicationStatusChangedEvent.eventType, PlacementEventListener.name)
  async handleApplicationStatusChangedEvent(
    event: JobApplicationStatusChangedEvent,
  ): Promise<void> {
    try {
      ...
      if (newStatus === "submitted") {
        ...
        const serviceType = getServiceTypeFromApplicationType(applicationType as ApplicationType);
        if (!serviceType) {
          this.logger.log(
  ...
  @HandlesEvent(JobApplicationStatusRolledBackEvent.eventType, PlacementEventListener.name)
    ...
    try {
      ...
      if (previousStatus === "submitted") {
        ...
        const serviceType = getServiceTypeFromApplicationType(
          jobApplication.applicationType as ApplicationType,
        );
