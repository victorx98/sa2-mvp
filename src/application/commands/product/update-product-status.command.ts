...
export class UpdateProductStatusCommand extends CommandBase {
   ...
   * @returns 更新后的产品
   */
  async execute(productId: string, targetStatus: ProductStatus): Promise<IProduct> {
    try {
      this.logger.debug(`Updating product status: ${productId} -> ${targetStatus}`);
      const product = await this.productService.updateStatus(productId, targetStatus);
      this.logger.debug(`Product status updated successfully: ${product.id} -> ${product.status}`);
      return product;
    } catch (error) {
