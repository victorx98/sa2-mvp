...
export class RegularMentoringCreatedEventHandler {
  ...
  @HandlesEvent(REGULAR_MENTORING_SESSION_CREATED_EVENT, 'ServicesModule')
  async handleSessionCreated(event: RegularMentoringSessionCreatedEvent): Promise<void> {
    ...
    try {
      ...
      this.eventBus.publish(
        {
          ...
          source: { domain: 'services', service: 'RegularMentoringCreatedEventHandler' },
        },
        'ServicesModule',
      );
      ...
      try {
        const session = await this.domainRegularMentoringService.getSessionById(payload.sessionId);
          ...
          this.eventBus.publish(
            {
              ...
              source: {
                domain: 'services',
                service: 'RegularMentoringCreatedEventHandler',
              },
            },
            'ServicesModule',
          );
  ...
  @HandlesEvent(REGULAR_MENTORING_SESSION_UPDATED_EVENT, 'ServicesModule')
  async handleSessionUpdated(event: RegularMentoringSessionUpdatedEvent): Promise<void> {
    ...
    try {
      if (!payload.meetingId) {
        ...
        this.logger.debug(
          `Updating meeting ${payload.meetingId} for session ${payload.sessionId}`,
        );
        ...
        if (!canUpdate) {
          ...
          await retryWithBackoff(
            async () => {
              await this.meetingManagerService.updateMeeting(payload.meetingId, {
          ...
          `);
          this.logger.debug(
            `Meetings table updated: topic=${payload.newTitle}, scheduleStartTime=${startTime.toISOString()}, scheduleDuration=${payload.newDuration}`,
          );
  ...
  @HandlesEvent(REGULAR_MENTORING_SESSION_CANCELLED_EVENT, 'ServicesModule')
