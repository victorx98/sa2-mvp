...
export class RegularMentoringService {
  ...
  async cancelSession(sessionId: string, reason: string) {
    ...
    try {
      ...
      // Step 3: Update calendar status (if needed)
      // TODO: Update calendar status based on session details

      this.logger.log(`Regular mentoring session cancelled: sessionId=${sessionId}`);
