    ...
    try {
      ...
      sessionResult = await this.db.transaction(async (tx: DrizzleTransaction) => {
        ...
        try {
          ...
          // Determine hostUserId based on meeting provider
          // - Feishu: requires Feishu open_id (ou_xxx...)
          // - Zoom: use "me" to use the authenticated Zoom account, or omit for default
          const hostUserId = provider === MeetingProviderType.FEISHU 
            ? FEISHU_DEFAULT_HOST_USER_ID 
            : undefined; // Zoom will use "me" as default
