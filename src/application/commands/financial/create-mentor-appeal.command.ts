...
export class CreateMentorAppealCommand extends CommandBase {
  ...
  async execute(input: any) {
    try {
      ...
      const appeal = await this.mentorAppealService.createAppeal(input, input.createdBy);
      this.logger.debug(`Mentor appeal created successfully: ${appeal.id}`);
      return appeal;
    } catch (error) {
      this.logger.error(`Failed to create mentor appeal: ${error.message}`, error.stack);
      throw error;
    }
  }
}
