...
describe('ServiceTypeService', () => {
  ...
  describe('search', () => {
    it('should return paginated service types successfully [应该成功返回分页的服务类型]', async () => {
      ...
      const mockServiceTypes = Array.from({ length: 5 }, () => generateMockServiceType());
      const total = 15;
      ...
      expect(mockRepository.findMany).toHaveBeenCalledWith(filter, pagination, sort);
    ...
    it('should use default pagination when not provided [当未提供分页时应该使用默认分页]', async () => {
      ...
      const mockServiceTypes = Array.from({ length: 20 }, () => generateMockServiceType());
      const total = 20;
      ...
      expect(mockRepository.findMany).toHaveBeenCalledWith(filter, undefined, undefined);
    });
  });
});
