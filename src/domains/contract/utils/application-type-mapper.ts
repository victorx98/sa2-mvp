 ...
 */
export const APPLICATION_TYPE_TO_SERVICE_TYPE_MAP: Record<ApplicationType, string> = {
 ...
 */
export function getServiceTypeFromApplicationType(applicationType: ApplicationType): string {
  return APPLICATION_TYPE_TO_SERVICE_TYPE_MAP[applicationType] || 'job_application';
}
...
export function isValidServiceType(serviceType: string): boolean {
  const validServiceTypes = new Set(Object.values(APPLICATION_TYPE_TO_SERVICE_TYPE_MAP));
