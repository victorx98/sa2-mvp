...
-- 添加 EXCLUDE 约束（先删除已存在的，再创建）
ALTER TABLE "calendar" DROP CONSTRAINT IF EXISTS "calendar_no_overlap";
...
-- 可以创建的外键（先删除已存在的，再创建）
ALTER TABLE "calendar" DROP CONSTRAINT IF EXISTS "fk_calendar_session";
