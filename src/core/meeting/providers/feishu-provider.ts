...
/**
 * Feishu Meeting Provider
 *
 * Implements IMeetingProvider interface for Feishu (Lark) platform
 */
@Injectable()
export class FeishuMeetingProvider implements IMeetingProvider {
  private readonly logger = new Logger(FeishuMeetingProvider.name);
  ...
  async createMeeting(input: ICreateMeetingInput): Promise<IMeetingInfo> {
    try {
      ...
      this.logger.debug(`Feishu API payload: ${JSON.stringify(payload)}`);
